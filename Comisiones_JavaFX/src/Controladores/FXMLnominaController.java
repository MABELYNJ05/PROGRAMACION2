...
public class FXMLnominaController implements Initializable {
    ...
    @FXML
    private TextField textRespuesta;
    @FXML
    ...
    private void eventKeyNum(KeyEvent event) {
        ...
        }else if(evt.equals(textFebrero)){
            if(!Character.isDigit(event.getCharacter().charAt(0))){
                event.consume();;
            }
        }else if(evt.equals(textMarzo)){
            if(!Character.isDigit(event.getCharacter().charAt(0))){
                event.consume();;
            }
        }else if(evt.equals(textCantidadBus)){
            if(!Character.isDigit(event.getCharacter().charAt(0))){
                event.consume();;
            }
    ...
    private void ButtonIngresoDatos(ActionEvent event) {
        ...
        vende.setNombre(textNombre.getText());
        vende.setEnero(Double.valueOf(textEnero.getText()));
        vende.setFebrero(Double.valueOf(textFebrero.getText()));
        vende.setMarzo(Double.valueOf(textMarzo.getText()));
        vende.setTotal(vende.getEnero()+vende.getFebrero()+vende.getMarzo());
        vende.setPromedio(vende.getTotal()/3);
        obj.escribir(vende);
